...
- [x] **録画支援**（企画を元に録画ガイド生成 → ステップ実行モードで撮影ガイド）
- [x] **編集支援**（AI編集提案: カット/テロップ/BGM/テンポ/ショート化 + FFmpeg: 無音検出/音量/縦動画/書き出し）
...
- 記事・SEOコンテンツを**量産**して製品を宣伝するための、**開発者(自分)専用**ツール。
- ナビでは「編集支援」と「投稿支援」の間に表示され、**開発モードのときだけ**現れます。
- 二重ゲート: バックエンドは開発モード以外で **403**、フロントは `dev_mode` のときのみナビ表示。
- **有効化**: バックエンド起動時に環境変数 `AIVC_DEV_MODE=1` を設定（または `.env` に記載）。
  ```bash
  # Windows PowerShell
  $env:AIVC_DEV_MODE = "1"; uvicorn app.main:app --port 8756
  ```
  既定（未設定）では無効なので、配布ビルドではエンドユーザーに露出しません。
